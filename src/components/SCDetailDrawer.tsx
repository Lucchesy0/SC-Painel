...
import { useSlaSettings, calculateSCReminderInfo } from '../utils/sla';
    ...
    <div className="fixed inset-0 z-50 flex justify-end bg-slate-900/50 dark:bg-slate-950/60 backdrop-blur-xs transition-opacity duration-200">
      <div className="bg-white dark:bg-[#1c2230] w-full max-w-xl h-full shadow-2xl flex flex-col overflow-hidden border-l border-slate-200 dark:border-slate-800 animate-in slide-in-from-right duration-200">
        ...
        <div className="p-4 sm:p-5 border-b border-slate-200 dark:border-slate-800 flex items-center justify-between bg-slate-50 dark:bg-[#151a26]">
          <div>
            ...
            <p className="text-xs text-slate-500 dark:text-slate-400 mt-0.5">
              Detalhes, itens solicitados e rastreabilidade na nuvem
            </p>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="flex-1 overflow-y-auto p-4 sm:p-5 flex flex-col gap-4">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-2.5">
            ...
            <div className="p-3 rounded-xl bg-slate-50 dark:bg-[#151a26] border border-slate-200 dark:border-slate-800 flex flex-col">
              ...
              <strong className="text-xs font-mono font-bold text-orange-600 dark:text-orange-400 mt-0.5 truncate">
                {reminder.dataVencimentoEfetiva ? formatDateBR(reminder.dataVencimentoEfetiva) : (sc.dataVencimento ? formatDateBR(sc.dataVencimento) : 'Sem prazo')}
              </strong>
            </div>
          </div>
          ...
          {/* Observações / Comentários da SC (se houver) */}
          {(sc.observacoes || sc.comentarios) && (
            <div className="p-3.5 rounded-xl bg-orange-50/50 dark:bg-orange-950/20 border border-orange-200 dark:border-orange-900/50">
              <span className="text-[11px] text-orange-700 dark:text-orange-300 font-bold flex items-center gap-1.5 mb-1">
                <MessageSquare className="w-3.5 h-3.5" /> Comentários / Observações
              </span>
              ...
              {sc.itens.length === 0 ? (
                ...
                sc.itens.map((item, idx) => {
                  ...
                  return (
                    <div
                      key={item.id || idx}
                      className="p-3.5 rounded-xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-[#181d28] shadow-xs flex items-center justify-between gap-3"
                    >
                      ...
                      <div className="shrink-0 text-right">
                        <span className="inline-flex items-center px-2.5 py-1 rounded-lg bg-slate-100 dark:bg-slate-800 font-mono text-xs font-bold text-slate-800 dark:text-slate-200 border border-slate-200/80 dark:border-slate-700">
                          {req} {item.unidade || 'UN'}
                        </span>
                      </div>
                    </div>
